...
def blog_index(request):
    posts_by_comment = Blog.objects.annotate(comment_count = Count('comments')).order_by('-comment_count')[:6]
    posts_by_time = Blog.objects.order_by('-created_at')
    
    return render(request,'blog/new.html',{'posts_by_comment':posts_by_comment,'posts_by_time':posts_by_time})
...
def get_blog_post(request,id):
    ...
    comments = Comment.objects.filter(blog_post = post,parent_comment__isnull=True)
    organized_comments = []

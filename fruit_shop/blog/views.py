...
def get_all_blogs(request):
    ...
    # queryset = Posts.objects.all()[offset:offset + items_per_page]
    # has_next_page = len(queryset) == items_per_page
    # return render(request, 'blog/blog.html', {'blog_posts': queryset, 'page_number': page_number, 'has_next_page': has_next_page})
...
#         blog = Posts.objects.create(title = title,
#                                  slug = slug,
#                                  subtitle = subtitle,
#                                  content = content,
#                                  author = request.user,
...
#     items_per_page = 4
#     offset = (int(page_number) - 1) * items_per_page
#     queryset = Posts.objects.filter(author=author)[offset:offset + items_per_page]
#     has_next_page = len(queryset) == items_per_page
...
#     blog_post = get_object_or_404(Posts, id=id)
...
#         Comments.objects.create(
#             blog_post = get_object_or_404(Posts,id=post_id),
#             author = request.user,
#             content = message,
#             is_approved = True,
...
#     blog_post = get_object_or_404(Posts, pk=post_id)
#     if request.method == 'POST':
...
#     if query:
#         blogs = Posts.objects.filter(title__icontains=query)
#     else:
#         blogs = Posts.objects.none()
...
#     passdef get_all_blog_posts(request):
#     page_number = request.GET.get('page', 1)
#     items_per_page = 4
#     offset = (int(page_number) - 1) * items_per_page 
#     queryset = Posts.objects.all()[offset:offset + items_per_page]
